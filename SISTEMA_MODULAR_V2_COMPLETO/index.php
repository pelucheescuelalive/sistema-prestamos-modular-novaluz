...
<head>
    ...
    <div class="main-content">
        ...
        <div id="tab-clientes" class="tab-content">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                ...
                <button class="btn btn-success" onclick="mostrarFormularioCliente()" style="font-size: 1.1em; padding: 12px 20px;">
                    ➕ Agregar Cliente
                </button>
            </div>
            
            <!-- Formulario oculto inicialmente -->
            <div class="form-container" id="form-container-cliente" style="display: none;">
        ...
        <div id="tab-prestamos" class="tab-content">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                ...
                <button class="btn btn-success" onclick="mostrarFormularioPrestamo()" style="font-size: 1.1em; padding: 12px 20px;">
                    ➕ Agregar Préstamo
                </button>
            </div>
            
            <!-- Formulario oculto inicialmente -->
            <div class="form-container" id="form-container-prestamo" style="display: none;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                    <h3>Crear Nuevo Préstamo</h3>
                    <button type="button" class="btn btn-secondary" onclick="ocultarFormularioPrestamo()">❌ Cancelar</button>
                </div>
    ...
    <script>
        ...
        // Funciones para el formulario de clientes
        function mostrarFormularioCliente() {
            document.getElementById('form-container-cliente').style.display = 'block';
            // Limpiar el formulario
            document.getElementById('form-cliente').reset();
            document.getElementById('cliente-id-edit').value = '';
            // Scroll suave hacia el formulario
            document.getElementById('form-container-cliente').scrollIntoView({ 
                behavior: 'smooth', 
                block: 'start' 
            });
        }
        ...
        function ocultarFormularioCliente() {
            document.getElementById('form-container-cliente').style.display = 'none';
            // Limpiar el formulario
            document.getElementById('form-cliente').reset();
            document.getElementById('cliente-id-edit').value = '';
        }
        
        // Funciones para el formulario de préstamos  
        async function mostrarFormularioPrestamo() {
            document.getElementById('form-container-prestamo').style.display = 'block';
            // Limpiar el formulario
            document.getElementById('form-prestamo').reset();
            // Cargar clientes primero y luego cargar en el select
            await cargarClientes();
            cargarClientesEnSelect();
            // Establecer fecha actual
            document.getElementById('prestamo-fecha').value = new Date().toISOString().split('T')[0];
            // Scroll suave hacia el formulario
            document.getElementById('form-container-prestamo').scrollIntoView({ 
                behavior: 'smooth', 
                block: 'start' 
            });
        }
        
        function ocultarFormularioPrestamo() {
            document.getElementById('form-container-prestamo').style.display = 'none';
            // Limpiar el formulario
            document.getElementById('form-prestamo').reset();
            // Limpiar calculadora
        ...
        // Función auxiliar para cargar clientes en el select de préstamos
        function cargarClientesEnSelect() {
            console.log('Iniciando cargarClientesEnSelect...');
            const select = document.getElementById('prestamo-cliente');
            console.log('Select encontrado:', select);
            // Limpiar opciones existentes
            select.innerHTML = '<option value="">Selecciona un cliente</option>';
            
            // Usar la variable global clientes cargada desde la API
            ...
            if (clientes && clientes.length > 0) {
                clientes.forEach(cliente => {
                    ...
                    option.textContent = cliente.nombre;
                    select.appendChild(option);
                    console.log('Cliente agregado al select:', cliente.nombre);
                });
            } else {
